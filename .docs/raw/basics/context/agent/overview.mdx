...
| `enable_agentic_knowledge_filters` | `bool`      | `False` | If True, let the agent choose the knowledge filters. See [knowledge](/basics/knowledge/filters/overview) for more information.                                                      |
...
If you are using a [Toolkit](/integrations/toolkits/overview) on your agent, you can add tool instructions to the system message using the `instructions` parameter:
...
If you have database storage enabled on your agent, session history is automatically stored (see [sessions](/basics/sessions/overview)).
...
See the [full example](/basics/context/agent/usage/filter-tool-calls-from-history) for a complete demonstration.
