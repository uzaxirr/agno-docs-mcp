...
keywords: [filter, filtering, qdrant]
---
...
<Steps>
  ...
  <Step title="Run Qdrant">
    ...
    ```bash
    docker run -d --name local-qdrant -p 6333:6333 qdrant/qdrant:latest
    ```
  </Step>
  ...
  <Step title="Run the example">
    <CodeGroup>
    ```bash Mac
    python cookbook/08_knowledge/filters/vector_dbs/filtering_qdrant_db.py
    ```
